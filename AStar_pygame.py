...
def Astar(gröse):
        while True:
            ...
            if (current.x, current.y) == (end.x, end.y):
                ...
                for node in path[:-2]:
                    y, x = node.y, node.x
                    pygame.draw.rect(screen, blue, (x*gröse+(x*1)+1, y*gröse+(y*1)+1, gröse, gröse))
                    pygame.display.update()
                    pygame.time.wait(30)
    ...
    for event in pygame.event.get():
        ...
        if pygame.mouse.get_pressed()[0]:
            try:
                ...
                if field[my][mx] == "O" and pygame.Surface.get_at(screen, pos) != black:
                    pygame.draw.rect(screen, black, (mx*gröse+(mx*1)+1, my*gröse+(my*1)+1, gröse, gröse))
                    field[my][mx] = "X"
    ...
    while z != fgröse - 1 and s != fgröse - 1:
        for z in range(len(field)):
            for s in range(len(field[z])):
                if field[z][s] == "S":
                    pygame.draw.rect(screen, green, (x, y, gröse, gröse))
                elif field[z][s] == "E":
                    pygame.draw.rect(screen, red, (x, y, gröse, gröse))
                else:
                    pygame.draw.rect(screen, white, (x, y, gröse, gröse))
